...
public class EtcdClusterSetup
{
    ...
    public async Task<EtcdClient> CreateClientToEtcdCluster()
    {
        await this._semaphoreSlim.WaitAsync();
        if (!this.IsClusterOn())
        {
            await this.ClusterSetup();
        }
    ...
    private bool IsClusterOn()
    {
        return this._etcdContainers.Count != 0;
    }
}